...
⚠️ 本番環境（Cloudflare）へデプロイする際の注意
本ソースコードには認証ロジックが組み込まれていません。そのまま本番環境にデプロイすると、WorkerのURLを知っている第三者にAIモデルを不正利用され、リソースの消費や意図しない課金が発生するリスクがあります。本番環境で運用する場合は、必ずAPIキー（Bear Token等）による認証ロジックを追加し、CORS制限を適切に行ってください。
